...
export default function DiscordChat({ currUser }: DiscordChatProps) {
  ...
  // Choose between global discord streams and the private/group Messenger
  const [subTab, setSubTab] = useState<"glob_discord" | "midy_messenger">("glob_discord");
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      {/* Sub tabs switches */}
      <div className="flex gap-2 bg-[#121214] p-1.5 rounded-2xl border border-white/5 w-fit">
        <button
          onClick={() => setSubTab("glob_discord")}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition cursor-pointer ${subTab === "glob_discord" ? "bg-purple-600 text-white shadow-lgKey" : "text-gray-400 hover:text-white"}`}
        >
          <Hash className="w-3.5 h-3.5" />
          <span>Discord Circle (Global)</span>
        </button>
        <button
          onClick={() => setSubTab("midy_messenger")}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition cursor-pointer ${subTab === "midy_messenger" ? "bg-purple-600 text-white shadow-lgKey" : "text-gray-400 hover:text-white"}`}
        >
          <MessageSquare className="w-3.5 h-3.5" />
          <span>Midy Messenger (PMs & GCs)</span>
        </button>
      </div>

      {subTab === "glob_discord" ? (
        <div className="bg-[#121214] border border-white/10 rounded-2xl p-6 flex flex-col md:flex-row h-[550px] shadow-2xl overflow-hidden select-none">
          ...
          <div className="w-full md:w-56 border-b md:border-b-0 md:border-r border-white/10 pb-3 md:pb-0 md:pr-3 flex flex-col">
            <div className="flex items-center gap-1.5 pb-2 mb-2 border-b border-white/10 block leading-tight">
              <MessageSquare className="w-4 h-4 text-purple-400" />
              <h3 className="font-extrabold text-[11px] text-white uppercase tracking-wider">MidYeah Channels</h3>
            </div>
        ...
        <MessengerChat currUser={currUser} />
      )}
    </div>
  );
}
